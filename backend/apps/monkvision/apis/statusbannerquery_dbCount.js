...
exports.doService = async jsonReq => {
    ...
    const truePercent = currentData[currentData.length-1].db_count;

    // calculate issue percentage
    const threshold = jsonReq["threshold"] || 5;
    ...
    if (threshold > truePercent) {fgcolor = jsonReq[ok_colorCode].split(",")[0], bgcolor = jsonReq[ok_colorCode].split(",")[1]}
    ...
    if (threshold > truePercent) textexplanation = "";
    else textexplanation = "";

    let add_symbol;
    if (!jsonReq.add_symbol){add_symbol="%"}
    else {add_symbol=jsonReq.add_symbol}
    const result = {result: true, type: "metrictext", contents: {textmain:`${truePercent} ${add_symbol}`, fgcolor, bgcolor, textexplanation}}; 
