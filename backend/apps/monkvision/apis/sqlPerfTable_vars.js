...
exports.doService = async jsonReq => {
    ...
    for (let row of rows) {
        ...
        for (let index = 0; index < data_array.length; index++) {
            let var_name = data_array[index].variable_name;
            let var_val = data_array[index].variable_val || "-";
    ...
    const result = { result: true, type: "table", contents: { length: x.length, x, ys: [y1, y2], infos: [[], []] } };
