...
exports.doService = async jsonReq => {
    ...
    for (let row of rows) {
        ...
        for (let index = 0; index < data_array.length-1; index++) {
            let Query_database = data_array[index].database;
            let Query_partitions = data_array[index].partitions;
            let Query_summary = data_array[index].summary;
            let Query_online = data_array[index].online;
            let Query_replicating = data_array[index].replicating;
            let Query_recovering = data_array[index].recovering;

            x.push(counter);
            y1.push(Query_database);
            y2.push(Query_partitions);
            y3.push(Query_summary);
            y4.push(Query_online);
            y5.push(Query_replicating);
            y6.push(Query_recovering);
